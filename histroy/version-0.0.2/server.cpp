		...
		for (i = 0; i < user_count + 1; i++) {
			if (fds[i].revents & POLLERR) {
				...
				if (siz < 0) {
					...
					printf("read %d bytes of Data [ %s ] from %s:%d\n", (int)siz,users[connfd].buf, 
                           inet_ntoa(users[connfd].address.sin_addr), 
                           ntohs(users[connfd].address.sin_port));
 					int j = 0;
